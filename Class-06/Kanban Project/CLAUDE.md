...
- Implemented as a modern NextJS app (App Router)
- The NextJS app should be created in a subdirectory `frontend`
- Persistence via Supabase (Postgres): single shared board, `columns` and `cards` tables
- Simple Supabase Auth (email + password) gates access to the board; no per-user boards, no profile management
